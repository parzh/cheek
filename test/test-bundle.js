...
describe("Bundle", function() {
	...
	describe("everyMethod(methodNames, input)", function() {
		it("returns `true` if all of the verifications return `true`", function() {
			assert(!cheek.everyMethod(["isNumber", "isFinite", "isPositive"], -5));
			assert(cheek.everyMethod(["isNumber", "isFinite", "isNegative"], -5));
			assert(cheek.everyMethod(["isString", "isPrimitive"], "my text"));
		});
	});

	describe("someMethod(methodNames, input)", function() {
		it("returns `true` if any of the verifications returns `true`", function() {
			assert(cheek.someMethod(["isNumber", "isString"], 42));
			assert(!cheek.someMethod(["isNotNatural", "isFloat"], 42));
			assert(!cheek.someMethod(["isObject", "isNull"], NaN));
		});
	});

	describe("everyInput(methodName, inputs)", function() {
		it("returns `true` if all input values pass verification", function() {
			assert(cheek.everyInput("isNumber", [5, 4, -2]));
			assert(!cheek.everyInput("isNumber", [5, 4, -2, "my text"]));
			assert(cheek.everyInput("isNatural", [1, 2, 3]));
		});
	});

	describe("someInput(methodName, inputs)", function() {
		it("returns `true` if any of the input values passes verification", function() {
			assert(cheek.someInput("isNotDefined", [5, 4, null, -2]));
			assert(!cheek.someInput("isObject", [5, 4, -2, "my text"]));
			assert(!cheek.someInput("isNaN", [1, 2, 3]));
		});
	});
});